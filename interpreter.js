class CossertInterpreter {
  ...
  prepare(source) {
    ...
    if (inComment) throw new Error("IWANNACOMENT never reached COMENT FINISH");
    return lines;
  }
  ...
  extractFunctions(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (i >= lines.length) throw new Error(`Function "${name}" forgot its }`);
  ...
  async execBlock(lines, parentIndent = -1) {
    ...
    while (i < lines.length) {
      ...
      if (t === "YO HOWSURDOIN 0.1") { i++; continue; }
      if (t === "OKALFINEBYE") return { signal: "end" };
      ...
      if (m) {
        ...
        const rhs = m[2].trim();
        if (rhs === '"nothing"' || rhs === "'nothing'" || rhs === "nothing") {
          this.vars[name] = this.emptyFor(this.vars[name]);
        } else {
          this.vars[name] = await this.evalExpr(rhs);
        }
        i++;
        continue;
      }
      ...
      if (m) return { signal: "return", value: await this.evalExpr(m[1]) };
  ...
  async evalExpr(expr) {
    ...
    if (m && this.functions[m[1]]) return await this.callFunction(m[1]);
    ...
    if (/^[A-Za-z_][A-Za-z0-9_]*$/.test(expr) && expr in this.vars) return this.vars[expr];
  ...
  splitComparison(expr) {
    for (const op of ["NO{=}", ">=", "<=", "==", ">", "<"]) {
      ...
      if (idx >= 0) {
        return { left: expr.slice(0, idx).trim(), op, right: expr.slice(idx + op.length).trim() };
      }
    }
    return null;
  }
  ...
  parseList(text) {
    ...
    catch { throw new Error(`Bad list: ${text}`); }
  }
